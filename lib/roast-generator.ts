...
export async function generateRoast(content: PortfolioContent): Promise<string> {
  ...
  // Detect common portfolio clichés only when present
  const portfolioCliches = [];
  ...
  if (hasTodoApp) portfolioCliches.push('todo app');
  if (hasWeatherApp) portfolioCliches.push('weather app');
  if (hasCalculator) portfolioCliches.push('calculator');
...
About: ${content.aboutMe}${portfolioCliches.length > 0 ? `
Portfolio Clichés Detected: ${portfolioCliches.join(', ')}` : ''}
...
- Reference current tech memes and debates (JavaScript fatigue, framework wars, etc.)${portfolioCliches.length > 0 ? `
- RUTHLESSLY mock the detected portfolio clichés: ${portfolioCliches.join(', ')} - these are bootcamp-level basic!` : ''}
...
- Call out any obvious bootcamp projects or tutorial follow-alongs
